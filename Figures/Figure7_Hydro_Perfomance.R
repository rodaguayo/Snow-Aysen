...
y2 <- list(titlefont = f, title = "SWE - R2 (mm)", tickfont = f2, ticks = "outside", zeroline = FALSE)

fig2 <- plot_ly(data_swe, showlegend = FALSE, y = ~RN2_case_1, x = ~date, type = 'scatter', mode = 'lines', name = "Case 1",  opacity  = 1, line = list(color = colors[2], width = 1))
fig2 <- fig2 %>% add_trace(y = ~RN2_case_2, mode = 'lines', name = "Case 2",  opacity  = 1, line = list(color = colors[4], width = 1))
fig2 <- fig2 %>% add_trace(y = ~RN2_case_3, mode = 'lines', name = "Case 3",  opacity  = 1, line = list(color = colors[6], width = 1))
fig2 <- fig2 %>% add_trace(y = ~RN2_case_4, mode = 'lines', name = "Case 4",  opacity  = 1, line = list(color = colors[8], width = 1))
fig2 <- fig2 %>% add_trace(y = ~RN2_case_5, mode = 'lines', name = "Case 5",  opacity  = 1, line = list(color = colors[10], width = 1))
fig2 <- fig2 %>% add_trace(y = ~RN2, mode = 'scatter', name = "Observed",  opacity  = 1, marker  = list(symbol = "x",color = "black", width = 1.5), line  = list(color = "black", width = 0.1))
...
htmlwidgets::saveWidget(as_widget(fig), "C:/Users/rooda/Dropbox/Proyectos/Semilla-Aysen/Figures/Figure7_Hydro.html")

server <- orca_serve()
server$export(fig, file = "C:/Users/rooda/Dropbox/Proyectos/Semilla-Aysen/Figures/Figure7_Hydro.png", width = 1000, height = 1000, scale = 3)
server$close()
