...
q_obs$r<- q_obs$q
q_obs$r[match(sample(q_obs$r, length(q_obs$r)*0.98), q_obs$r)] <- NA
...
KGE_df<-data.frame(matrix(ncol = 9, nrow = 6))
...
swe_route_p$RN1_case_1<-model_1$swe[,1]
...
out <- hydroPSO(fn="hydromodInR", lower=lower_param,  upper=upper_param, method="spso2011", model.FUN="TUWhydromod",
                control=list(write2disk=FALSE, MinMax="max", npart=40, maxit=100, normalise=TRUE, REPORT=5, reltol=1E-6),
...
swe_route_p$RN1_case_2<-model_2$swe[,1]
...
swe_route_p$RN1_case_3<-model_3$swe[,1]
...
swe_route_p$RN1_case_4<-model_4$swe[,1]
...
TUWhydromod <- function(param.values, obs=q_obs$q, prec=pp, airt=t2m, ep=pet, area=areas) {
  ...
  gof1      <- KGE(sim=c(model_5$swe[,1], model_5$swe[,2], model_5$swe[,3], model_5$swe[,8]), 
                   obs=c(swe_route_p$RN1, swe_route_p$RN2, swe_route_p$RN4, swe_route_p$RN7), method="2012", na.rm=TRUE)
  gof2      <- KGE(model_2$q, obs, method="2012", na.rm=TRUE)
  
  out       <- vector("list", 2)   # Creating the output of the R function
  out[[1]]  <- mean(gof1, gof2)
  out[[2]]  <- model_5$q
  names(out) <- c("GoF", "sim")  # Mandatory names for the elements of the output 
...
swe_route_p$RN1_case_5<-model_5$swe[,1]
...
write.csv(q_obs,  "C:/Users/rooda/Dropbox/Proyectos/Semilla-Aysen/Results/q_results.csv")
