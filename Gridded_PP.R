...
pp_data_mod <-as.data.frame(t(extract(pp_era5, pp_shape)))
colnames(pp_data_mod)<-as.data.frame(pp_shape)$name
pp_data_mod <- pp_data_mod[-1, ]
pp_data$date <-NULL

KGE(pp_data_mod, pp_data, method="2012", out.type = "full")


## Bias correction using El Fraile CIEP
qmap_power<-fitQmapPTF(pp_data[,7], pp_data_mod[,7], transfun="linear", wet.day=TRUE, cost="RSS")
